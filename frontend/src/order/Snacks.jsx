...
const Snacks = () => {
  ...
  const fetchSnacks = async () => {
    try {
      ...
      const now = new Date();
      const snackItems = res.data.filter(
        (item) =>
          item.type === "snacks" &&
          item.ordered === false &&
  ...
  const handleOrder = async (id) => {
    if (!user) {
      alert("Please login to order snacks.");
      return;
    }
    ...
    try {
      ...
      alert("Snacks ordered successfully! Check 'My Orders'.");
      fetchSnacks();
    } catch (err) {
      console.error(err);
    ...
    <div className="bg-orange-50 min-h-screen py-24 px-6">
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          <p className="text-gray-500 text-xl font-medium">No snacks available right now.</p>
        ...
        <div className="max-w-6xl mx-auto glass-card rounded-2xl overflow-hidden border border-white/50 shadow-xl">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse" ref={tableRef}>
              <thead className="bg-gradient-to-r from-yellow-50 to-yellow-100 text-yellow-900 uppercase text-xs font-bold tracking-wider">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100 bg-white/60">
                {foods.map((food) => (
                  <tr
                    key={food._id}
                    className="food-row hover:bg-yellow-50/50 transition-colors duration-200"
                  >
                    {/* IMAGE & NAME */}
